...
const triples = (()=>{
    ...
    // Skip native binary on win32/arm64 - it often fails to load; use WASM fallback instead
    if (PlatformName === "win32" && ArchName === "arm64") {
        return [];
    }
...
async function loadBindings(useWasmBinary = false) {
    ...
    pendingBindings = new Promise(async (resolve, _reject)=>{
        ...
        try {
            ...
            // When native fails for any reason (e.g. Windows ARM64 binary load error), try WASM fallback
            try {
                let wasmFallbackBindings = await tryLoadWasmWithFallback(attempts);
                if (wasmFallbackBindings) {
                    return resolve(wasmFallbackBindings);
                }
            } catch (wasmAttempts) {
                if (Array.isArray(wasmAttempts)) attempts = attempts.concat(wasmAttempts);
            }
        }
